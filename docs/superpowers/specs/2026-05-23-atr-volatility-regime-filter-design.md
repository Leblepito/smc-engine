...
risk_guard.validate(setup, account_state, config)
  ...
  │    → averaging → drawdown_breaker → time-gate (funding|session)
...
```python
# build_with_diagnostics icinde, basarili setup yapilirken (mevcut diagnostics
# scope'unda — Setup donmeden hemen once Setup kwarg'ina yansir):
...
if h4 is not None and getattr(h4, "atr_history", None):
    ...
    if len(history) >= window // 2:
        ...
        diagnostics["atr_percentile"] = rank  # mevcut diagnostics scope'unda

# Setup constructor'ina yansit
setup = Setup(..., regime_metrics=regime_metrics)
```
...
class Setup:
    ...
    #                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # Python mutable default footgun'unu engelle (default_factory zorunlu).
    # Eski Setup(...) construction'lari kwarg vermeden calismaya devam eder
...
5. **`test_build_with_diagnostics_writes_atr_percentile_to_setup`**
   - H4 snapshot `atr_history=[1.0, 2.0, ..., 100.0]` (100 elem), current_atr=80,
     window=96 → recent = [5..100], count(v≤80) = 76, rank ≈ 0.792
   - `Setup.regime_metrics["atr_percentile"] == pytest.approx(0.792, rel=0.02)`
...
`_check_volatility_regime` checks listesinin **sonuna** eklenir
(cheap, deterministik, diğer gate'lerin başarısızlık reason'unu maskelemesin).
Diğer gate'ler erken filtreliyor; vol gate'i "geride kalan kaliteli setup'ı
rejim nedeniyle reddet" demek. Bu spec'in commit'i — planner alternatif
sıralamayı tartışmaz.
...
- [ ] Eski `Setup(...)` constructor çağrıları `regime_metrics` kwarg vermeden
      çalışmaya devam eder (geriye uyumluluk — `field(default_factory=dict)`)
- [ ] Eski `TFSnapshot(...)` constructor çağrıları `atr_history` kwarg vermeden
      çalışmaya devam eder (default `None`)
- [ ] En az 1 integration test: orchestrator.analyze → setup_builder →
      risk_guard.validate end-to-end, sentetik yüksek-vol H4 verisi
      `volatility_regime` rejection üretir (8 unit testin yakalayamadığı
      wiring bug'ı buradan görünür)
- [ ] CLI `--atr-percentile-threshold 0.70,0.80,0.90` mevcut
      `_parse_float_list` ile uyumlu parse eder
